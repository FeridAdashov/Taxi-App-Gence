...
public class LabelPlacement {
    ...
    static final Logger log = LoggerFactory.getLogger(org.oscim.layers.tile.vector.labeling.LabelPlacement.class);
    ...
    boolean updateLabels(LabelTask work) {
        ...
        //TODO Changed Codes remove Armenian words
        for (Label ti = mLabels; ti != null; ti = (Label) ti.next) {
            if (ti.label != null && !checkAzerbaijanAlphapet(ti.label))
                if (ti.label.length() > 15)
                    ti.label = "QARABAĞ AZƏRBAYCANDIR";
                else ti.label = "";
    ...
    private boolean checkAzerbaijanAlphapet(String text) {
        String alphabet = "abcdefghijklmnopqrstuvwxyzöğışçüİABCDEFGHIJKLMNOPQRSTUVWXYZÖĞÇŞI0123456789";
        String alphabet2 = "ԱաԲբԳգԴդԵեԶզԷէԸըԹթԺժԻիԼլԽխԾծԿկՀհՁձՂղՃճՄմՅյՆնՇշՈոՉչՊպՋջՌռՍսՎվՏտՐրՑցՒւՓփՔքՕօՖֆուև";
        for (char ch : text.toCharArray())
            if (alphabet2.contains(String.valueOf(ch))) return false;
        return true;
    }
