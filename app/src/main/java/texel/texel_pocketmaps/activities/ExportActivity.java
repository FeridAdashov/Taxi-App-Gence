...
import texel.texel_pocketmaps.downloader.MapUnzip;
import texel.texel_pocketmaps.downloader.ProgressPublisher;
...
public class ExportActivity extends AppCompatActivity implements OnClickListener, OnItemSelectedListener, OnCheckedChangeListener {
    ...
    public static FileType getFileType(String file) {
        ...
        if (Variable.isSavingFile(file)) {
            return FileType.Setting;
        }
        if (file.endsWith(".gpx")) {
            return FileType.Tracking;
        }
        if (file.endsWith("Favourites.properties")) {
            return FileType.Favourites;
        }
    ...
    public void onClick(View v) {
        if (v.getId() != R.id.exOk) { // Import a pmz file.
    ...
    private void exportMaps(File zipBaseFile) {
        ...
        for (View dView : lMaps.getTouchables()) {
            ...
            if (fullSize > freeSize) {
                logUser("Out of disk-memory");
                return;
            }
            for (String mFile : mDir.list()) { // No directory allowed
