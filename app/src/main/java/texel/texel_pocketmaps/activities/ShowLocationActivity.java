...
public class ShowLocationActivity extends AppCompatActivity {
    ...
    private void handleInput() {
        ...
        try {
            ...
            if (!getLocationFromQueryParameters(prop, uri)
                    && !getLocationFromParsingParameters(prop, uri)
                    && !getLocationFromParsingValues(prop, uri))
            ...
            e.printStackTrace();
            logUser("Yanlış daxiletmə: " + e.getMessage());
        }
    }
    ...
    private boolean getLocationFromParsingValues(Properties prop, Uri uri) {
        ...
        if (latLon.length != 2 && latLon.length != 3) {
            return false;
        } // lat,lon[,alt]
    ...
    private boolean checkLocationParameters(Properties prop) {
        ...
        if (prop.get("lat") == null) {
            return false;
        }
        return prop.get("lon") != null;
    }
    ...
    private void showMain() {
        ...
        intent.putExtra("texel.pocketmaps.activities.PocketMaps.MapActivity.SELECTNEWMAP", false);
