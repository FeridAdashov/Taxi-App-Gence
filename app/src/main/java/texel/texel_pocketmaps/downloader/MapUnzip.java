...
import texel.texel_pocketmaps.activities.ExportActivity;
import texel.texel_pocketmaps.activities.GeocodeActivity;
...
public class MapUnzip {
    ...
    private static boolean checkUnzipAliveOldVersion(Context c, MyMap myMap) {
        ...
        if (timeCheck(mDir.lastModified())) {
            return true;
        }
        File[] list = mDir.listFiles();
        if (list != null) {
            for (File f : list) {
                if (timeCheck(f.lastModified())) {
                    return true;
                }
            }
        }
    ...
    public boolean unzipImport(final String zipFilePath, final Context appContext) {
        ...
        try {
            ...
            if (ExportActivity.getFileType(entry.getName()) == ExportActivity.FileType.Map) { // Is a map file!
        ...
        try {
            ...
            for (int i = 0; i < srcFiles.size(); i++) {
                ...
                if (!zipSubDir.isEmpty()) {
                    zipName = new File(zipSubDir, zipName).getPath();
                }
                if (pp != null) {
                    fcount++;
                    pp.updateText(false, "" + fcount + "/" + flen + " Export " + zipName, 0);
                }
                File f = new File(curFile);
                long bcount = 0;
                long bLen = 4000;
                if (f.exists()) {
                    fis = new FileInputStream(new File(curFile));
                    bLen = f.length();
                } else {
                    fis = context.openFileInput(curFile);
                }
                ...
                while (true) {
                    ...
                    if (pp != null) {
                        int per = (int) ((bcount * 100) / bLen);
                        pp.updateText(true, "" + fcount + "/" + flen + " Export " + zipName, per);
                    }
            ...
            if (pp != null) {
                pp.updateTextFinal("Tamamlandı: Export " + new File(tarZipFile).getName());
            }
        } catch (IOException e) {
            e.printStackTrace();
            IOUtils.closeQuietly(fis);
            if (pp != null) {
                pp.updateTextFinal("Səhv baş verdi: Export " + new File(tarZipFile).getName());
            }
            return false;
        } finally {
    ...
    private void extractFile(ZipInputStream zipIn,
        ...
        try {
            if (appContextWhenInternal == null) {
                bos = new BufferedOutputStream(new FileOutputStream(filePath));
            } else {
                bos = new BufferedOutputStream(appContextWhenInternal.openFileOutput(filePath, Context.MODE_PRIVATE));
            }
            byte[] bytesIn = new byte[BUFFER_SIZE];
            int read = 0;
            long readCounter = 0;
