package texel.texel_pocketmaps.model;
...
public class MyMap implements Comparable<MyMap> {
    ...
    public static File getMapFile(MyMap myMap, MapFileType fileType) {
        ...
        } else if (fileType == MapFileType.MyFile) {
            return new File(dlFolder, "MapDownloadStatus");
        } else // MapFolder
    ...
    public void checkUpdateAvailableMsg(final Activity activity) {
        ...
        new AsyncTask<Void, Void, MyMap>() {
            ...
            @Override
            protected void onPostExecute(MyMap myMapRemote) {
    ...
    public String getTime(boolean ofRemote) {
        ...
        if (timeLocal.isEmpty()) {
            timeLocal = readTimeLocal(getMapName());
        }
        return timeLocal;
    }
    ...
    public enum MapFileType {DlMapFile, MapFolder, DlIdFile, MyFile}
