...
import texel.texel_pocketmaps.util.LightSensor;
import texel.texel_pocketmaps.util.SpeedUtil;
...
public class NaviEngine {
    ...
    protected UiJob uiJob = UiJob.Nothing;
    ...
    public void setNavigating(Activity activity, boolean active) {
        this.active = active;
        if (active && lightSensor == null) lightSensor = new LightSensor(activity);
...
//        if (uiJob != UiJob.RecalcPath) { throw new IllegalStateException("Getting instructions but state is not RecalcPath!"); }
        if (!directTargetDir) {
            nearestP.checkDirectionOk(pos, instructions.get(0));
        }
                ...
                if (in == null) {
                    if (NaviEngine.this.uiJob == UiJob.RecalcPath) {
                        if (instructions != null) {
                            instructions = null;
                            MapHandler.getMapHandler().calcPath(recalcFrom.getLatitude(),
                                    recalcFrom.getLongitude(), recalcTo.getLatitude(),
                                    recalcTo.getLongitude(), activity, null, null, null);
    ...
    private void setTiltMult(double nextDist) {
        double speedXtra = 0;
        if (pos != null) {
            speedXtra = pos.getSpeed();
        }
        if (speedXtra > 30.0) {
            speedXtra = 2;
        } else if (speedXtra < 8.0) {
            speedXtra = 0;
        } else {
    ...
    private NaviInstruction calculatePosition(GeoPoint curPos) {
        ...
        if (nearestP.arrPos > 0) { // Check dist to line (backward)
        ...
        if (nearestP.arrPos < instructions.get(0).getPoints().size() - 1) { // Check dist to line (forward)
            double lat1 = instructions.get(0).getPoints().getLatitude(nearestP.arrPos);
            double lon1 = instructions.get(0).getPoints().getLongitude(nearestP.arrPos);
            double lat2 = instructions.get(0).getPoints().getLatitude(nearestP.arrPos + 1);
            double lon2 = instructions.get(0).getPoints().getLongitude(nearestP.arrPos + 1);
            ...
            if (lDist < nearestP.distance) {
                ...
                instructions.size() > 1) {
            if (instructions.get(1).getPoints().size() > 0) { // Check dist to line (forward to next instruction)
                double lat1 = instructions.get(0).getPoints().getLatitude(nearestP.arrPos);
                double lon1 = instructions.get(0).getPoints().getLongitude(nearestP.arrPos);
            ...
            if (instructions.get(1).getPoints().size() > 1) { // Check dist to line (forward next instruction p1+p2)
        ...
        if (!nearestP.isDistanceOk()) {
            ...
            if (directTargetDir) {
                maxWayTolMeters = maxWayTolMeters * 10.0;
            }
            ...
            if (nearestNext == null) {
                ...
                return null;
            } else { // Forward to nearest instruction.
                int deleteCounter = 0;
                ...
                if (lastDeleted != null) { // Because we need the current, and not the next Instruction
    ...
    private NaviInstruction getNewInstruction() {
        ...
        if (instructions.size() > 0) {
            ...
            naviVoiceSpoken = speakDistanceCheck(in.getDistance()) && nearestP.isDirectionOk();
            return nIn;
        }
    ...
    static class PointPosData {
        ...
        public boolean isBackward() {
            return (status == Status.CurPosIsBackward);
        }

        public boolean isForward() {
            return (status == Status.CurPosIsForward);
        }

        public boolean isForwardNext() {
            return (status == Status.CurPosIsForwardNext);
        }
        ...
        public void checkDirectionOk(Location pos, Instruction in) {
        ...
        private void calculateWrongDir(Location pos, Instruction in) {
            ...
            if (!wrongDir) {
                ...
                wrongDir = (bearingDiff > 50); //TODO This value was 100
