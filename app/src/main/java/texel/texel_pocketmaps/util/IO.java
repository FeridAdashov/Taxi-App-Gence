...
public class IO {
    ...
    public static long dirSize(File dir) {
        ...
        long result = 0;
        for (File file : dir.listFiles()) { // Recursive call if it's a directory
            if (file.isDirectory()) {
                result += dirSize(file);
            } else {
                result += file.length();
    ...
    public static File getDefaultBaseDirectory(Context context) {
        if (Build.VERSION.SDK_INT >= 29) { // ExternalStoragePublicDirectory Deprecated since android Q
            ...
            if (!Environment.getExternalStorageState(target).equals(Environment.MEDIA_MOUNTED)) {
                Toast.makeText(context, "Xəritə daxili yaddaşa icazə olmadan işlək deyil!", Toast.LENGTH_SHORT).show();
                return null;
            }
            return target;
        } else if (Build.VERSION.SDK_INT >= 19) { // greater or equal to Kitkat
            if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                Toast.makeText(context, "Xəritə xarici yaddaşa icazə olmadan işlək deyil!", Toast.LENGTH_SHORT).show();
                return null;
            }
    ...
    private static ArrayList<String> listSelectionPathsLollipop(Context context, boolean cacheDir) {
        ...
        for (File curFile : files) {
            if (curFile == null) { // Regarding to android javadoc this may be possible.
                continue;
            }
    ...
    private static String getStorageText(File dir) {
        ...
        return "[ " + freeSpace + " MB boş / " + totalSpace + " MB ümumi]\n" + dir.getPath() + "\n";
    }
}
